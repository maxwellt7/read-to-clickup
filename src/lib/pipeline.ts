...
export async function processMeeting(meetingId: string): Promise<PipelineResult> {
  ...
  try {
    ...
    if (isLowConfidence) {
      ...
      fallbackListId = await findOrCreateList(fallbackFolderId, FALLBACK_ROUTE.listName);
      console.warn(
        `Low confidence (${confidence.toFixed(2)}) for meeting ${meetingId}. ` +
        `Classified as "${call_type}" — also stored in Needs Review.`,
      );
    }
    ...
    // Also write to Needs Review if low confidence
    if (fallbackListId) {
    ...
    console.error(`[pipeline] Failed for meeting ${meetingId}:`, err);
    return { success: false, meetingId, error };
  }
}
